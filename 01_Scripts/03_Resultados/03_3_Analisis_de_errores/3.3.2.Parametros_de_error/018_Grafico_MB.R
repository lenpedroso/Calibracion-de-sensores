...
# 1. Gráfico de barras  MB  ------------------------------------------------
barras_MB <- ggplot(Errores_antes_despues,
                    aes(x = Sensor, y = MB, fill = Clasificación)) +
  geom_bar(
    stat = "identity",
    color = "black", # añade un borde negro a cada grafica
    position = position_dodge(width = 0.9)
  ) +
  geom_errorbar(
    aes(ymin = MB - SD_MB, ymax = MB + SD_MB),# añade los valores de desviación del error
    width = 0.2,
    linewidth = 0.8,
    position = position_dodge(width = 0.9)
  ) +
  ...
  theme(
    ...
    legend.title = element_blank() # Elimina el titulo en la leyenda
  ) +
  labs(title = "MB antes / después de calibrar", x = "Sensor", y = "MB (µg/m³)")
